...
export default function App() {
  ...
  // --- AUTH ---
  const handleAuthSubmit = async (user, pass, isRegister, email = "") => {
    try {
      const endpoint = isRegister ? "register/" : "login/";
      const payload = isRegister ? { username: user, email, password: pass } : { username: user, password: pass };
      ...
      <div className="w-1/2 bg-white flex items-center justify-center p-12">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={(e) => {
            ...
            const email = isRegister ? e.target[1].value : "";
            const pass = isRegister ? e.target[2].value : e.target[1].value;
            handleAuthSubmit(user, pass, isRegister, email);
          }} className="space-y-4">
            <div>
